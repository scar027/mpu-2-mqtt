...
#define I2C_FREQ_HZ 400000  // Typical I2C clock speed
...
typedef void *mpu6050_handle_t; // New mpu6050_handle_t
...
// Additional structs to support dual mpu6050 setup
typedef struct {
    mpu6050_acce_value_t acce_1;
    mpu6050_acce_value_t acce_2;
} dual_acce_value_t;

typedef struct {
    mpu6050_gyro_value_t gyro_1;
    mpu6050_gyro_value_t gyro_2;
} dual_gyro_value_t;

// Add handle pair struct and functions
typedef struct {
    mpu6050_handle_t mpu1;
    mpu6050_handle_t mpu2;
} mpu6050_handle_pair_t;
...
/**
* @brief Create a pair of MPU6050 sensor handles
* @param bus_handle Initialized I2C bus handle
* @param dev_addr1 First sensor address (typically 0x68)
* @param dev_addr2 Second sensor address (typically 0x69)
* @return Handle pair object
*/
mpu6050_handle_pair_t mpu6050_create_pair(i2c_master_bus_handle_t bus_handle, 
                                        uint16_t dev_addr1, 
                                        uint16_t dev_addr2);

/**
* @brief Get dual sensor data in one atomic operation
* @param pair Handle pair from mpu6050_create_pair()
* @param acce Accelerometer data structure for both sensors
* @param gyro Gyroscope data structure for both sensors
* @return ESP_OK on success
*/
esp_err_t mpu6050_get_dual_data(mpu6050_handle_pair_t pair,
                               dual_acce_value_t *acce,
                               dual_gyro_value_t *gyro);

/**
* @brief Delete a sensor pair
* @param pair Handle pair to delete
*/
